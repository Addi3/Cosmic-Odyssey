...
execute at @e[tag=vortextardis1] if entity @p[distance=..10] if score invortex tardis1 matches 1 run title @p actionbar [{"score":{"objective":"tardis1","name":"vortextimer"},"color":"gold"},{"text":" / 10000","color":"gold"}]
...
execute if score rotorsfxtimer tardis1 matches 65.. run scoreboard players set rotorsfxtimer tardis1 0
...
execute if score rotorsfxtimer tardis1 matches 1 at @e[tag=vortextardis1] if entity @p[distance=..5] run playsound minecraft:vortex ambient @p ~ ~ ~ 5 1 1
